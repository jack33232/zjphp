 ...
 * DataCollection
 *
 * DataCollection Class based on Klein DataCollection Class but extends
 * Component class with the ability to have behavior and event
 *
 * Inspired by @fabpot's Symfony 2's HttpFoundation
 * @link https://github.com/symfony/HttpFoundation/blob/master/ParameterBag.php
 */
class DataCollection extends Component implements IteratorAggregate, ArrayAccess, Countable
{
     ...
     */
    public function __construct(array $attributes = array(), array $config = [])
    {
        if (!empty($attributes)) {
    ...
    {
        return parent::__isset($key) || $this->exists($key);
    }
    ...
    public function __unset($key)
    {
        if ($this->exists($key)) {
            $this->remove($key);
        } elseif ($this->canSetProperty($key)) {
            parent::__unset($key);
        }
    }
